...
# --- Write client code (no GUI, terminal only) ---
cat > "$INSTALL_DIR/client.py" << 'PYEOF'
import json, os, platform, random, socket, string, subprocess, sys, threading, time
import websocket
...
class DeployClient:
    def __init__(self):
        ...
        print("  ╚══════════════════════════════════╝")
        print(f"  设备: {self.sys_info['os']} / {self.sys_info['arch']} / {self.sys_info['hostname']}")
        print("  按 Ctrl+C 退出")
        print("")
        print_status("正在连接...")
    ...
    def _ws_loop(self):
        while self.should_run:
            ...
            if self.should_run:
                self.ws_connected = False
                print_status("连接断开，3秒后重连...")
                time.sleep(3)

    def _on_ws_open(self, ws):
        self.ws_connected = True
        print_status("已连接 - 等待指令")
    ...
    def _exec(self, task_id, cmd):
        ...
        if self.ws_connected: print_status("已连接 - 等待指令")
